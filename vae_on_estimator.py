...
def get_df_batch_from_bigquery(project_id, dataset, table, min_rnum, max_rnum) :
  min_rnum += 1
  max_rnum += 1
...
def static_nlog_dataset(project, dataset, table, start, end):
  data_arr = get_df_batch_from_bigquery(project, dataset, table, start, end)
  data_arr = data_arr[data_arr != np.array(None)]
  label_arr = np.ones(shape=data_arr.shape)
...
def build_input_fns(data_dir, batch_size):
  ...
  training_dataset = static_nlog_dataset('bi-service-155107', 'bigpi_test', 'log_anomaly_globalsignin_devicemodel_20181004_real', 0, 100000)
  ...
  eval_dataset = static_nlog_dataset('bi-service-155107', 'bigpi_test', 'log_anomaly_globalsignin_devicemodel_20181004_test_shuffled', 0, 1000)
